...
export const SESSION_SECONDS = 60 * 60 * 24 * 35;
...
const bytesBuffer = (value: Uint8Array) =>
  value.buffer.slice(
    value.byteOffset,
...
export const validRecoveryEmail = (email: string) =>
...
export async function hashPassword(password: string) {
  ...
  const key = await crypto.subtle.importKey(
    "raw",
    bytesBuffer(textBytes(password)),
    "PBKDF2",
    false,
    ["deriveBits"],
  );
...
export async function verifyPassword(password: string, encoded: string) {
  ...
  const bits = await crypto.subtle.deriveBits(
    {
      name: "PBKDF2",
      salt: bytesBuffer(base64UrlToBytes(saltText)),
...
export async function verifyTotp(secret: string, code: string) {
  ...
  for (const drift of [-1, 0, 1])
    if ((await totpCode(secret, current + drift)) === code.trim()) return true;
  return false;
}

export async function signJwt(claims: Claims, secret: string) {
  const header = bytesToBase64Url(
    textBytes(JSON.stringify({ alg: "HS256", typ: "JWT" })),
  );
  const payload = bytesToBase64Url(textBytes(JSON.stringify(claims)));
  const key = await crypto.subtle.importKey(
    "raw",
    bytesBuffer(textBytes(secret)),
  ...
  const signature = await crypto.subtle.sign(
    "HMAC",
    key,
    bytesBuffer(textBytes(`${header}.${payload}`)),
  );
...
export async function readSession(
  ...
  try {
    ...
    return claims.exp > Math.floor(Date.now() / 1000) &&
      (!claims.purpose || claims.purpose === "session")
      ? claims
      : null;
  } catch {
    return null;
...
export const sessionCookie = (token: string) =>
  `${SESSION_COOKIE}=${token}; Max-Age=${SESSION_SECONDS}; Path=/; HttpOnly; Secure; SameSite=Lax`;
