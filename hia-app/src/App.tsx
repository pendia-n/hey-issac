...
function Shop({ setPage, notify }: { setPage: (page: Page) => void; notify: (message: string) => void }) {
	const [siteUrl, setSiteUrl] = useState(''); const [objective, setObjective] = useState(''); const [tier, setTier] = useState('starter'); const [addon, setAddon] = useState('default'); const [message, setMessage] = useState(''); const [running, setRunning] = useState(false); const [balance, setBalance] = useState<number | null>(null); const [result, setResult] = useState<any>(null);
	useEffect(() => { fetch('/api/billing', { credentials: 'include' }).then((response) => response.ok ? response.json() : null).then((body) => setBalance(body?.balanceCents ?? null)); }, []);
	const run = async () => { setMessage(''); setResult(null); setRunning(true); const response = await fetch('/api/runs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ siteUrl, objective: objective || 'Find the most useful next growth actions.', tier, addon, idempotencyKey: crypto.randomUUID() }) }); const body = await response.json(); setRunning(false); if (!response.ok) return setMessage(body.error ?? 'The run could not be completed.'); setResult(body); notify('Your evidence-backed list is ready.'); };
	return <section className="page setup"><div className="eyebrow">Shop for growth</div><h1>Bring back a useful list.</h1><p className="lede">Give heyIssac a public website and one thing you want to understand. It will search, read, diagnose, and rank the next steps.</p><div className="panel form-panel"><div className="field"><label htmlFor="site">Your website</label><input id="site" type="url" placeholder="https://yourbusiness.com" value={siteUrl} onChange={(event) => setSiteUrl(event.target.value)} /><p className="hint">Only public pages are checked.</p></div><div className="field"><label htmlFor="objective">What should we look at?</label><textarea id="objective" rows={3} placeholder="Example: why are beginner classes hard to find?" value={objective} onChange={(event) => setObjective(event.target.value)} /></div><div className="choice-row"><label className="field"><span>Plan model</span><select value={tier} onChange={(event) => setTier(event.target.value)}><option value="starter">Starter</option><option value="studio">Studio</option><option value="partner">Partner</option></select></label><label className="field"><span>Run upgrade</span><select value={addon} onChange={(event) => setAddon(event.target.value)}><option value="default">Included model</option><option value="push">Push</option><option value="max">Max</option></select></label></div><button className="primary" disabled={running} onClick={run}>{running ? 'Working through the evidence…' : 'Make my list'} <span>→</span></button>{balance !== null && <p className="hint">Usage balance: ${(balance / 100).toFixed(2)}. Push and Max use this balance.</p>}{message && <p className="form-error" role="alert">{message}</p>}</div>{result && <article className="panel finding run-result"><span className="tag easy">Run complete</span><h2>{result.result?.summary ?? 'Your findings are ready.'}</h2><p>Model cost: ${(result.providerCostUsd ?? 0).toFixed(4)}. Charged: ${((result.chargedCents ?? 0) / 100).toFixed(2)}. Evidence sources: {result.evidenceCount ?? 0}.</p>{(result.result?.actions ?? []).map((action: any) => <div className="action" key={action.title}><div><h3>{action.title}</h3><p>{action.rationale}</p></div><span className="tag important">{action.priority ?? 'next'}</span></div>)}</article>}</section>;
}
