...
type AuthSession = { sub: string; username: string; role: string; iat: number; exp: number; workspaceId?: string };
const getWorkspaceForUser = (db: D1Database, userId: string, workspaceId?: string) => db.prepare('SELECT workspace_id FROM workspace_members WHERE user_id = ? AND (? IS NULL OR workspace_id = ?) ORDER BY workspace_id LIMIT 1').bind(userId, workspaceId ?? null, workspaceId ?? null).first<{ workspace_id: string }>();
const getWorkspaceContext = (db: D1Database, userId: string, workspaceId?: string) => db.prepare('SELECT w.id, w.plan, w.subscription_status, w.stripe_subscription_id, w.current_period_end, w.cancel_at_period_end FROM workspaces w JOIN workspace_members m ON m.workspace_id = w.id WHERE m.user_id = ? AND (? IS NULL OR m.workspace_id = ?) ORDER BY w.created_at LIMIT 1').bind(userId, workspaceId ?? null, workspaceId ?? null).first<{ id: string; plan: string; subscription_status: string; stripe_subscription_id?: string | null; current_period_end?: string | null; cancel_at_period_end?: number }>();
...
app.get('/api/auth/tokens', async (c) => {
	const session = await getSession(c); if (!session) return apiError(c, 'Sign in required', 401);
	const rows = await c.env.DB.prepare('SELECT id, name, expires_at, revoked_at, created_at, last_used_at FROM api_tokens WHERE user_id = ? AND (? IS NULL OR workspace_id = ?) AND revoked_at IS NULL ORDER BY created_at DESC').bind(session.sub, session.workspaceId ?? null, session.workspaceId ?? null).all(); return c.json({ tokens: rows.results });
});
app.post('/api/auth/tokens', async (c) => {
	...
	const workspace = await getWorkspaceForUser(c.env.DB, session.sub, session.workspaceId); if (!workspace) return apiError(c, 'Workspace not found.', 404);
...
app.delete('/api/auth/tokens/:id', async (c) => {
	const session = await getSession(c); if (!session) return apiError(c, 'Sign in required', 401); const result = await c.env.DB.prepare('UPDATE api_tokens SET revoked_at = ? WHERE id = ? AND user_id = ? AND (? IS NULL OR workspace_id = ?) AND revoked_at IS NULL').bind(new Date().toISOString(), c.req.param('id'), session.sub, session.workspaceId ?? null, session.workspaceId ?? null).run(); return result.meta.changes === 1 ? c.json({ ok: true }) : apiError(c, 'Token not found.', 404);
});
app.get('/api/projects', async (c) => {
	const session = await getSession(c); if (!session) return apiError(c, 'Sign in required', 401); const workspace = await getWorkspaceForUser(c.env.DB, session.sub, session.workspaceId); if (!workspace) return apiError(c, 'Workspace not found.', 404); const projects = await c.env.DB.prepare('SELECT id, site_url, product_category, icp, target_geo, created_at FROM projects WHERE workspace_id = ? ORDER BY created_at DESC').bind(workspace.workspace_id).all(); return c.json({ projects: projects.results });
});
app.post('/api/projects', async (c) => {
	const session = await getSession(c); if (!session) return apiError(c, 'Sign in required', 401); const workspace = await getWorkspaceForUser(c.env.DB, session.sub, session.workspaceId); if (!workspace) return apiError(c, 'Workspace not found.', 404);
...
app.post('/api/billing/subscription/cancel', async (c) => {
	const session = await getSession(c); if (!session) return apiError(c, 'Sign in required', 401);
	const workspace = await getWorkspaceContext(c.env.DB, session.sub, session.workspaceId); if (!workspace) return apiError(c, 'Workspace not found.', 404);
